...
- ✅ **Sprint 6 — Vessel padronizado + polish visual** — fechada em 2026-05-07. Branch `feature/sprint6-vessel-catalog`, 8 commits atômicos (50-58). Identificado via screenshot QMoor do user: vessel deve ser catálogo padronizado (dropdown), não input livre, com preview live no editor e SVGs dedicados por tipo. Backend: tabela `vessel_types` com 9 vessels canônicos (P-77, FPSO Suezmax, Atlanta, Semisub genérico, Spar, AHV 200te/100te, Drillship, Barge MODU); 5 endpoints REST `/vessel-types` com 403 para seed canônica (mesmo padrão F6/F1a). Frontend: `VesselPicker` reusa padrão BuoyPicker; `VesselEditor` refatorado com picker + badge 'do catálogo'/'manual'; rastreabilidade via `vessel.catalog_id` (não-autoritativo). Fix preview: CaseFormPage passa `vessel` para CatenaryPlot do PlotPanel (bug reportado pelo user). Plot: 3 SVGs novos (semisubSvg, fpsoSvg, sparSvg) + dispatcher `getVesselSvg`; AHV deck mid-line (Sprint 5) refinado para SVG image laranja. Suite: 954→971 backend (+17) + 207 frontend. Zero regressão F0-F11 + Sprints 1-5. **Decisão fechada #20**. Pendências v1.4+: catálogo expandido (Damen/ULSTEIN/Bourbon); 3D casco com perspectiva; auto-orientação; multi-vessel. Ver [`docs/relatorio_sprint6_vessel_catalog.md`](docs/relatorio_sprint6_vessel_catalog.md).
- ✅ **Sprint 5 — AHV Tier D Operacional Mid-Line** — fechada em 2026-05-07. Branch `feature/sprint5-ahv-operational`, 8 commits atômicos (42-49). 4º cenário AHV (após Sprint 2 instalação, Sprint 4 Tier C endpoint, Fase 8 carga pontual): linha de ancoragem instalada continua íntegra; AHV puxa lateralmente via Work Wire conectado num ponto intermediário (mid-line). Schema novo: `LineAttachment.ahv_work_wire: Optional[WorkWireSpec]` + `ahv_deck_x` — default None preserva F8 puro. Solver `backend/solver/ahv_operational.py` (~340 LoC) com pre-processor 2-pass + refinamento iterativo (convergência |ΔX|+|ΔZ| < 0.5m em até 5 ciclos). Fallback automático F8 com D025 (info, high). D026 (warning) ww com ângulo vertical < 10°. D018 atualizado com tier_d_active=True. Parser QMoor detecta `boundary.anchorHandlerVessels[]`. Frontend: AHVOperationalSubcard no AttachmentsEditor. CatenaryPlot desenha linha laranja entre pega → AHV deck. Validação: 6 BC-AHV-OP-01..06 vs MoorPy marcados xfail informativos (divergência conceitual pega-FREE vs pega-FIXA). Suite: 941 → 954 backend + 207 frontend. Zero regressão. **Decisão #19**. Ver [`docs/relatorio_sprint5_ahv_operational.md`](docs/relatorio_sprint5_ahv_operational.md).
- ✅ **Sprint 4 — AHV Tier C físico com MoorPy Subsystem** — fechada em 2026-05-06. Branch `feature/sprint4-ahv-tier-c`, 9 commits atômicos (33-41) + hotfix `fix/buoy-pendant-line-picker` em main. **Substitui Decisão #17** com **Decisão #18**: Tier C validado contra MoorPy Subsystem (open-source NREL, peer-reviewed ASME 2025) ao invés de exigir QMoor reference data. Solver `backend/solver/ahv_work_wire.py` (~520 LoC) resolve catenárias acopladas (mooring + Work Wire elástico) via brentq sobre Z_p com continuidade horizontal no ponto de pega. **Fallback automático Sprint 2** quando regime degenerado (mooring 100% apoiado em águas rasas com bollard moderado — caso real de instalação) com D024 (info, high) transparente. Validação: 10 BC-AHV-MOORPY canônicos — 6 ativos (todos via fallback Sprint 2 fisicamente correto), 4 xfail informativos: 2 uplift+touchdown imediato (pendência F7.x), 2 deepwater taut com divergência ~20% em H vs MoorPy (pendência F-prof.X — refatorar elastic.py para alinhar com mass distribuída). AHV + uplift single-seg destravado (`endpoint_grounded=False + work_wire`). Diagnostics novos: D022 (warning, high) — bollard pull ≥ 90% MBL Work Wire (DNV-OS-E301); D024 (info, high) — fallback Sprint 2 transparente. D018 atualizado com parâmetro `tier_c_active` que customiza mensagem citando snap loads + hidrodinâmica como fora do escopo. Parser QMoor 0.8.0 detecta `boundary.workWire` opcional e popula `AHVInstall.work_wire` (D023). Frontend: `WorkWireSubcard` colapsado no `AHVInstallEditor` com toggle "Ativar Tier C" + LineTypePicker + 6 campos físicos. `CatenaryPlot` desenha Work Wire em laranja (`#F97316`, dashdot) via `SolverResult.work_wire_start_index`. Hotfix paralelo: `LineTypePicker` no campo "Modelo do cabo" do pendant das boias (UX inconsistente F5.7 corrigida). Suite: 921 backend (+52 testes Sprint 4) + 207 frontend (+3 WorkWireSubcard). Zero regressão F0-F11 + Sprints 1-3. cases_baseline_regression 3/3 + BC-MOORPY 7/7 + KAR006 16/16 preservados rtol=1e-9. Pendências v1.2: F7.x uplift+touchdown; F-prof.X catenária deepwater; multi-seg + Tier C; regen openapi.ts com `work_wire_start_index`. Ver [`docs/relatorio_sprint4_ahv_tier_c.md`](docs/relatorio_sprint4_ahv_tier_c.md).
- ✅ **Sprint 3 — Pendências v1.1.x: iteração automática + openapi regen** — fechada em 2026-05-06. Branch `feature/v1-1-x-pending`, 3 commits atômicos (30, 31, 32). **Atende 2 das 3 pendências post-Sprint 2** (a terceira — modelagem física AHV Tier C — adiada com justificativa). Pendência #1 (Commit 30): helper `frontend/src/lib/ahvIteration.ts` com `iterateBollardPullForTargetX(case, targetX, options)` — bissection client-side com bracket adaptativo. Etapa 1: encontra bracket [lo, hi] tal que x(lo) < target < x(hi); inicial [bollard/4, bollard×4], expande até 5× dobrando. Etapa 2: bissection (12 iter default) com tolerância 0.5m. Aceita callback `onStep` para feedback progressivo. Retorna `IterationResult` com `{converged, bollardPullFinal, xResultFinal, errorFinal, steps, stopReason}`; stopReason: tolerance_met | max_iters | bracket_invalid. UI inline progress no `AHVInstallEditor`: estado React local (running, steps, result), botão "Aplicar via Sensitivity" troca para spinner, sub-componente `IterationProgress` mostra tabela compacta (iter/bollard te/X m/erro/status) com highlight da melhor linha (verde) e badge de status (Loader2/CheckCircle2/XCircle), aplica bollard_pull final via `setValue` e emite toast.success/warning/error. Cada iteração chama `/solve/preview` (~50ms), total ~500ms-1s. 4 smoke tests com previewSolve mocked (bissection convergente, callback onStep, bracket_invalid, expansão adaptativa). Pendência #3 (Commit 31): regen `frontend/src/types/openapi.ts` via `npx openapi-typescript http://127.0.0.1:8765/api/v1/openapi.json -o ...` (backend FastAPI rodado localmente). Tipos novos reconhecidos: `AHVInstall`, `Vessel`, `CurrentProfile`/`CurrentLayer`, `PendantSegment`, `BoundaryConditions.ahv_install`, `CaseInput.{vessel, current_profile, metadata}`. Cleanup: 4 casts `as unknown as {...}` em `CaseDetailPage` removidos (acesso direto agora). Schema Zod `pendant_segments[].category` alinhado: `z.string()` → `z.enum(['Wire', 'StuddedChain', 'StudlessChain', 'Polyester'])` (era mismatch silencioso antes do regen). Restantes 9 casts em outros arquivos (AttachmentsTable, SolverDiagnosticsCard, CatenaryPlot internos) mantidos — referenciam campos NÃO no openapi (diagnostics, segment_boundaries) ou narrow intencional (StartpointType literal). **Pendência #2 (Tier C físico AHV) ADIADA para v1.2.0+** com justificativa: sem QMoor reference numbers para validar, qualquer modelo "novo" seria especulação não-validada. Quando user tiver acesso ao QMoor (ou outro software validado) e puder passar 3-5 cases-validados-por-ele (input + output esperado), iniciamos. Princípio de decisão fechada #15 (Sprint 2): solver core inalterado até ter dados de validação. Suite: 200 frontend (era 196, +4 ahv-iteration smokes), zero regressão backend (863 passed). Ver [`relatorio_sprint3_v1_1_x.md`](docs/relatorio_sprint3_v1_1_x.md).
- ✅ **Sprint 2 — AHV Install scenarios (v1.1.x)** — fechada em 2026-05-06. Branch `feature/ahv-install-ui`, 8 commits atômicos (23, 24, 24.1, 25, 26, 27, 28, 29). **Destrava 5 cases AHV de instalação (Backing Down, Hookup, Load Transfer) que falhavam com "fsolve não convergiu para multi-segmento + slope" no KAR006**. Causa raiz: cenários AHV em mode Range com L_total < L_min(X, h) — catenária 2D estática não fecha. QMoor real internamente usa modo dirigido por bollard pull (força do cabo de trabalho); `horzDistance` é informativo. Estratégia escolhida pelo usuário: **Opção C.2 completa** (schema rico + UI editável + plot adaptado). Schema novo `AHVInstall` em `backend/solver/types.py` com `bollard_pull` (req), `deck_level_above_swl`, `stern_angle_deg`, `target_horz_distance` (informativo). Anexado a `BoundaryConditions.ahv_install: Optional[AHVInstall]` via `model_rebuild()` (forward ref). Parser `moor_qmoor_v0_8.py` detecta cenário AHV por DOIS sinais: (a) `boundary.startpointType="AHV"` explícito; (b) nome do mooringLine contém "Hookup"/"Backing Down"/"Load Transfer"/"Install". Quando detectado: força `mode=Tension`, popula `ahv_install` com `bollard_pull = solution.fairleadTension OU heurística max(50 te, 1.5·w_max·h)`, preserva `horzDistance` em `ahv_install.target_horz_distance`. Diagnostic D021 (info) emitido no migration log. Solver core INALTERADO — caminho Tension já validado em F1+. Frontend: `AHVInstallEditor` componente com 4 campos (warning/30 tom amarelo distintivo), nova aba "AHV Install" no CaseFormPage (badge "1" quando setado), `ImportedModelCard` ganha bloco AHV Install com Bollard Pull (kN+te) / Target X / Deck / Stern, `CatenaryPlot` desenha linha vertical pontilhada amarela (#F59E0B) em x=target_horz_distance com hover info ("X resultante depende do bollard pull aplicado"). Tests: 13 schema (`test_ahv_install_schema.py`) + 9 BC-AHV-INSTALL (`test_ahv_install_scenarios.py`) + 4 frontend smokes. Diferença vs `LineAttachment.kind="ahv"` (Fase 8): aquele = carga pontual aplicada NUM PONTO da linha; este (`AHVInstall`) = "fairlead virtual" durante instalação (substitui rig fairlead pelo convés do AHV). Coexistem. **Resultado em produção: 16/16 KAR006 cases convergem** (era 11/16). Suite: 863 backend + 196 frontend (era 854+192, +9 backend +4 frontend, zero regressão). Baseline gate 3/3 + BC-MOORPY 7/7 + BC-SLACK-01 preservados rtol=1e-9. Pendências v1.1.x+: iteração automática "Aplicar via Sensitivity" (botão para encontrar bollard que dá X=target); modelagem física AHV completa com Work Wire elástico separado (Tier C, requer validação contra QMoor); openapi.ts regen. Ver [`docs/relatorio_sprint2_ahv_install.md`](docs/relatorio_sprint2_ahv_install.md).
- ✅ **Sprint 1 — Import QMoor 0.8.0 (v1.1.0)** — fechada em 2026-05-06. Branch `feature/qmoor-import-sprint1`, 10 commits atômicos. **Estratégia Tier A: schema rico, cálculo simples** — solver core inalterado, foco em destravar import 100% fiel de modelos profissionais (KAR006-like). Schemas novos: (1) `CaseInput.metadata` operacional (rig/location/region/engineer/number, ≤20 chaves); (2) `LineAttachment.pendant_segments[]` com `PendantSegment` lenient (só `length` obrigatório, ≤5 trechos); (3) `CaseInput.vessel: Optional[Vessel]` **case-level metadata, NÃO `kind="vessel"`** — decisão consciente porque QMoor JSON tem vessels como top-level (não dentro de mooringLines), conceitualmente diferente de carga aplicada num ponto da linha; (4) `CaseInput.current_profile` (`CurrentLayer` + `CurrentProfile` 1..20 layers ordenadas, drag_coefficient/water_density opcionais). Helper opt-in `discretize_current_profile()` em `backend/solver/current_discretizer.py` — Morison estático `F_i = 0.5·ρ·Cd·D·Δs·V²` (defaults ρ=1025, Cd=1.2), **NÃO auto-invocado** pelo solver — caller decide quando converter perfil em AHVs pontuais. Parser `backend/api/services/moor_qmoor_v0_8.py`: mapping `mooringLines[] × profiles[] → list[CaseInput]` cartesiano, profile_filter callable, log estruturado de truncamento/fallback (>10 segs, >20 layers, kind desconhecido → 'buoy'). 2 endpoints REST `/import/qmoor-0_8/{preview,commit}` + UI selector tabular em ImportExportPage com detecção automática. `ImportedModelCard` read-only no CaseDetailPage exibindo vessel + corrente V(z) (barras horizontais por depth) + metadata + badges source_*. **Editor manual de vessel/current/metadata é Tier B (post-v1.1.0)** — Sprint 1 entrega DISPLAY apenas. E2E `preview → commit → solve → export` + invariante crítico: case com vessel/current/metadata produz mesmo resultado de case manual sem esses campos (rtol=0). Fixtures sintéticos `qmoor_v0_8_synthetic.py` com KAR006-like (3 cases). KAR006 real auto-pickup quando arquivo está em `docs/audit/qmoor_kar006_sample.json` (skipa caso contrário). Suite: 820 backend + 187 frontend (era 665+181, +155 backend +6 frontend, zero regressão F1-F11). Baseline gate: 3/3 cases preservam resultado rtol=1e-9. Pendências post-Sprint: editor manual; discretização auto via UI button (com D020 idealização); export QMoor 0.8.0 round-trip; openapi.ts regen (tipos inline em ImportedModelCard como ponte temporária). Ver [`docs/relatorio_sprint1_qmoor_import.md`](docs/relatorio_sprint1_qmoor_import.md).
