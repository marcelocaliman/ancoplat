...
- ✅ **Fase 7** — fechada em 2026-05-05. Anchor uplift (suspended endpoint): solver agora habilita `endpoint_grounded=False` para single-segmento sem attachments via novo módulo `backend/solver/suspended_endpoint.py` (catenária livre PT_1 fully suspended, espelha MoorPy); função `_solve_uplift_tension_mode` aceita `s_a<0` (vértice em "U") — diferente da versão upstream que rejeita; `BoundaryConditions.endpoint_depth: Optional[float]` com validação Pydantic cruzada (4 cenários inválidos rejeitados); dispatcher no facade `solve()` delega single-seg+sem attachments+uplift; multi-seg+uplift e attachments+uplift levantam `NotImplementedError` específico (Q3=b reserva F7.x). 5 BC-UP-01..05 vs MoorPy local com erro real ≤ 0.25% (gate 1e-2, folga 4×); **2 BC-MOORPY-04/05 destravados** (skipados desde F1) → 9 BC-MOORPY ativos. D016 (uplift fora de domínio) + D017 (uplift desprezível < 1m sugere voltar a grounded). Frontend: radio Grounded/Suspended + input endpoint_depth condicional + plot deslocando anchor por endpoint_depth + sample/verbete glossário destravados. Convenção MoorPy descoberta: `Catenary.catenary` retorna `(FxA, FzA, FxB, FzB, info)` — anchor PRIMEIRO. F7 é feature completa sem ressalva (≠ F8). 610 backend + 2 skipped + 105 frontend verdes. Ver [`relatorio_F7_anchor_uplift.md`](relatorio_F7_anchor_uplift.md).
- ✅ **Fase 8** — fechada em 2026-05-05. AHV (Anchor Handler Vessel): `LineAttachment.kind="ahv"` aplica carga estática horizontal num ponto da linha durante operação de instalação. **Idealização modelada explícita** documentada em 3 níveis: D018 (warning, sempre dispara em AHV — Q6 não-negociável), Memorial PDF com seção "AHV — Domínio de aplicação" + texto literal verificado via 5 strings-chave (Q9 + Ajuste 3), manual de usuário pendência F11. Schema 4 campos (`ahv_bollard_pull` + `ahv_heading_deg` em referencial X global anti-horário + 2 metadados); Pydantic cross-validation cruzado. Solver multi-segmento estendido: `_signed_force_2d` retorna (H_jump, V_jump); `_integrate_segments` agora trata H per-segmento (mudança crítica — era constante); `_solve_suspended_tension` ajusta H_fairlead = H_anchor + sum_H_jump. **4 BC-AHV-01..04 vs cálculo manual com erro 0.0000%** (catenária paramétrica é exata; sem MoorPy externo). D019 (Ajuste 1) alerta quando heading resulta em projeção <30% no plano da linha (evita confusão de "AHV digitado mas força fora do plano"). AHV + uplift bloqueado com mensagem clara; AHV + boia/clump em multi-seg suportado (Q5). Frontend: Select kind +"AHV", campos Bollard pull/Heading condicionais, sample `ahv-pull` destravado (BC-AHV-01 funcional), 2 verbetes glossário destravados. **Paridade total com QMoor alcançada**. 666 backend + 2 skipped + 119 frontend verdes. Ver [`relatorio_F8_ahv.md`](relatorio_F8_ahv.md).
- ✅ **Fase 10** — fechada em 2026-05-05. V&V completo (gate de release v1.0). 11 commits atômicos + 1 fix. Catálogo **VV-01..14** unificado em `backend/solver/tests/test_vv_v1.py` com erro real medido por caso serializado em `docs/audit/vv_v1_errors.json`. **Watchcircle paralelizado com ProcessPoolExecutor** (Spread 4× 56s→16.6s, 3/4 cenários <20s). p95 endpoints <100ms (5/5). Round-trip unidades 59 testes rtol<1e-10. Robustez R1..R8 (8/8 sem crash). Apply tests 100% diagnostics (5 garantidos + 6 xfail informativos). Cobertura 96% agregado (gap 98% críticos honestamente documentado em [`relatorio_F10_C6_coverage.md`](relatorio_F10_C6_coverage.md)). Suite: 665 backend + 6 xfail + 181 frontend verdes. Pendências v1.1 não-bloqueantes: BC-UP-06..10 + BC-AHV-05..10 (lista detalhada no relatório), VV-07/08 via MoorPy Subsystem, watchcircle shallow chain heurística pré-fsolve. Ver [`relatorio_F10_vv_completo.md`](relatorio_F10_vv_completo.md) e [`relatorio_VV_v1.md`](relatorio_VV_v1.md).
- ⬜ Fase 11 — pendente (manual de usuário com seção AHV obrigatória + release notes v1.0 + checklist final).
